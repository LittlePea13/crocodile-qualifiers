...
class WikiDataJSONLDataReader:
    ...
    def read_documents(self):
        ...
        for i,j,y in os.walk(self.dataset_file):
            for file_name in y:
                iter_doc = open(i + '/' + file_name, "r")
                for elem in iter_doc:
                    elem = json.loads(elem)
                    document = Document(
                        docid=elem.get('id'),
                        pageuri=elem.get('wikidata'),
                        title=elem.get('title'),
                        text=elem.get('text'),#.decode('utf-8')
                    )
                    for elem_child in elem['links']:
                        entity = Entity(elem_child.get('wikidata'),
                                        boundaries=elem_child.get('boundaries'),
                                        surfaceform=elem_child.get('label'),
                                        title=elem_child.get('title'),
                                        annotator='Me')
                        document.entities.append(entity)
